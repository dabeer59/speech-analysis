...
# # media deps (guarded so import errors don't crash the app)
# try:
#     from pydub import AudioSegment
# except Exception:
#     AudioSegment = None
...
# os.environ["HF_HOME"] = "/tmp/hf"  # slim cache on Streamlit Cloud
# try:
...
# # 🔐 hardcoded as requested (replace with your real key)
...
# def load_model_and_processor():
#     def _load(mid):
#         proc = AutoProcessor.from_pretrained(mid, target_lang=LANG_CODE)
...
#             try:
#                 mdl.load_adapter(LANG_CODE)
#             except Exception:
#                 pass
#         mdl.eval()
#         return proc, mdl, mid

#     # try big, then small (handles OOM / quota differences across accounts)
#     try:
...
#         clip.audio.write_audiofile(output_path, fps=16000, logger=None, verbose=False)
#         clip.close()
#     elif ext in [".mp3", ".aac", ".flac", ".ogg", ".m4a"]:
#         if AudioSegment is None:
#             raise RuntimeError("pydub not available. Install pydub and ffmpeg.")
#         audio = AudioSegment.from_file(file_path)
#         audio = audio.set_frame_rate(16000).set_channels(1)
#         audio.export(output_path, format="wav")
#     elif ext == ".wav":
#         if AudioSegment is None:
#             # fallback path using soundfile if pydub isn't present
#             data, sr = sf.read(file_path, dtype="float32", always_2d=False)
#             if isinstance(data, np.ndarray) and data.ndim > 1:
#                 data = data.mean(axis=1)
#             if sr != 16000:
#                 data = librosa.resample(data, orig_sr=sr, target_sr=16000)
#             sf.write(output_path, librosa.util.normalize(data).astype(np.float32), 16000)
#         else:
#             audio = AudioSegment.from_wav(file_path)
#             audio = audio.set_frame_rate(16000).set_channels(1)
#             audio.export(output_path, format="wav")
...
# # ---------------- Core functions ----------------
# def transcribe(wav_path) -> str:
#     # lazy load happens here on first call
#     processor, model, which = load_model_and_processor()
#     audio, sr = librosa.load(wav_path, sr=16000, mono=True)
#     inputs = processor(audio, sampling_rate=sr, return_tensors="pt", padding=True)
#     with torch.no_grad():
#         logits = model(**inputs).logits
...
#         system_instruction=system_instr
#     )
#     resp = model.generate_content(transcript, generation_config={"temperature": 0.0})
#     # defend against None in some SDK responses
...
#             wav_path = get_wav_from_input(temp_path)
#         except Exception as e:
#             st.error(f"Conversion failed: {e}. Ensure ffmpeg/pydub/moviepy are installed.")
#             st.stop()
#         transcript = transcribe(wav_path)
